...
void OSPMainDialog :: setSignals(){
	...
	connect(ui->mvUp,SIGNAL(released()),this,SLOT(upReleased()));
	connect(ui->mvDown,SIGNAL(released()),this,SLOT(downReleased()));
	connect(ui->mvRight,SIGNAL(released()),this,SLOT(rightReleased()));
	connect(ui->mvLeft,SIGNAL(released()),this,SLOT(leftReleased()));
    //  button for sending x,y steps for motor movement
    connect(ui->nSteps,SIGNAL(clicked()),this,SLOT(nStepClicked()));       
    ...
    connect(ui->StepAdj,SIGNAL(clicked()),this,SLOT(adjToggled()));    
...
//     	device.stop();
// }
void OSPMainDialog :: upReleased(){
        
       if(ui->StepAdj->isChecked()) {
         device.movy(1);
        }
        else device.stop();
}

void OSPMainDialog :: downReleased(){
        if(ui->StepAdj->isChecked()) {
         device.movy(0);
        }
        else device.stop();
...
void OSPMainDialog :: rightReleased(){
        if(ui->StepAdj->isChecked()) {
         device.movx(1);
        }
        else device.stop();
...
void OSPMainDialog :: leftReleased(){
...
void OSPMainDialog :: nStepClicked(){

    QString sx , sy ;
    QString x = ui->xSteps->text();  
    QString y = ui->ySteps->text(); 
       
    double xi = x.split(" ")[0].toDouble();             
    double yi = y.split(" ")[0].toDouble();

    device.stepx(xi,yi) ;

    //update X,Y coordinates in UI
    QString s=QString::number(xi + ui->X->text().toDouble());
    QString t=QString::number(yi + ui->Y->text().toDouble());
    ui->X->setText(s);             
    ui->Y->setText(t);           
...
void OSPMainDialog :: adjToggled(){
        ...
        else if(ui->FineAdj->isChecked()){
            device.FineAdj();
            qDebug() << "[OpenSkyPlanetarium]:Low Speed ON" << endl;
        }
        else {                                // step adjust is checked         
           
            device.StepAdj();
            qDebug() << "[OpenSkyPlanetarium]:Step adjust is ON" << endl;
        }
