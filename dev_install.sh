...
before_reboot(){
	...
	printf 'y\n' | sudo apt-get install g++ gcc wget
	...
	if [ ${MACHINE_TYPE} == 'x86_64' ]; then
		...
		export PATH">> ~/.bashrc
		echo "/usr/local/Qt-5.7.0/bin/usr/local/Qt-5.7.0/lib">>/usr/lib/i386-linux-gnu/qt-default/qtchooser/default.conf             
	...
	printf 'y\n' | sudo apt-get install build-essential cmake zlib1g-dev libgl1-mesa-dev gcc g++  \
	     qtscript5-dev libqt5svg5-dev qttools5-dev-tools qttools5-dev libqt5opengl5-dev qtmultimedia5-dev \
...
if [ -f /var/run/rebooting-for-updates ]; then
    ...
    echo "Restart required. Do you want to continue? [Y/n]."
    read input
